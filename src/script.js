...
function getContrastColor(hex) {
    // ⚡ Bolt: Memoize color contrast calculations
    // Impact: Avoids redundant string manipulation and math
    ...
    if (hex && hex.startsWith('#')) {
        ...
        if (r !== undefined && g !== undefined && b !== undefined) {
            ...
            result = (yiq >= 140) ? '#1a1a1a' : '#ffffff'; // 140 threshold, using dark gray for black for softer look
        }
    }

    if (getContrastColor.cache.size > 1000) {
        getContrastColor.cache.clear();
    }
...
function hexToRgba(hex, alpha) {
    // ⚡ Bolt: Memoize hex to rgba conversion
    // Impact: Avoids redundant string parsing
    ...
    if (hexToRgba.cache.size > 1000) {
        hexToRgba.cache.clear();
    }
...
function parseDateString(str) {
    ...
    // ⚡ Bolt: Memoize expensive regex operations
    // Impact: Avoids redundant regex matching for recurring dates
    ...
    if (parseDateString.cache.has(str)) {
        ...
        return cached ? { ...cached } : null; // Return shallow clone to prevent mutation
    }
    ...
    if (parseDateString.cache.size > 1000) {
        parseDateString.cache.clear();
    }
