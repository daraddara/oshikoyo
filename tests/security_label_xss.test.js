...
describe('Security: XSS Vulnerability in renderCalendar label', () => {
    ...
    it('should be fixed and escape event type label', () => {
        ...
        // 1. Verify no <img> element with 'onerror' is present (XSS mitigation)
        const xssImg = dayCell.querySelector('img[onerror]');
        expect(xssImg).toBeNull();

        // 2. Verify the title attribute contains the literal payload (decoded by JSDOM)
        const oshiEvent = dayCell.querySelector('.oshi-event');
        expect(oshiEvent.getAttribute('title')).toContain(xssPayload);
    });
});
