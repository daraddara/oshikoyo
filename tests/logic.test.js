...
describe('祝日判定ロジック (getJPHoliday)', () => {
    it('2024年の祝日を正しく判定できること', () => {
        const { getJPHoliday } = loadModule(
            [`const appSettings = { externalHolidays: {} };`, holidayLogic],
            ['getJPHoliday']
        );
    ...
    it('外部APIデータがある場合に優先されること', () => {
        const { getJPHoliday } = loadModule(
            [`const appSettings = { 
                externalHolidays: { "2024-01-01": "スーパー元日" } 
            };`, holidayLogic],
            ['getJPHoliday']
        );
        expect(getJPHoliday(new Date(2024, 0, 1))).toBe('スーパー元日');
    });

    it('外部APIデータがない場合は既存ロジックへフォールバックすること', () => {
        const { getJPHoliday } = loadModule(
            [`const appSettings = { 
                externalHolidays: { } 
            };`, holidayLogic],
    ...
    it('2020年・2021年のオリンピック特例の移動を正しく判定できること', () => {
        ...
        expect(getJPHoliday(new Date(2021, 7, 8))).toBe('山の日');
